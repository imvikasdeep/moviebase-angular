...
export class MoviesService {
    ...
    getMovies(movie_category: string, params: any): Observable<IMovieList> {
        ...
        if (movie_category == 'genre')
            API_URL = `${this.API_URL}discover/movie`;

        if (movie_category == 'search')
            API_URL = `${this.API_URL}search/movie`;
    ...
    getMovieGenres(): Observable<{ genres: IGenre[] }> {
        return this.http.get<{ genres: IGenre[] }>(`${this.API_URL}genre/movie/list`).pipe(map((response: { genres: IGenre[] }) => {

            response.genres.forEach(ele => {
                ele.slug = this.slugify(ele.name)
            })
